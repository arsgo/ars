...
func (s *ScriptPool) getImageDrawTypeBinder() script.LuaTypesBinder {
...
func typeNewDraw(L *lua.LState) int {
	...
	if count > 2 {
		...
		ud.Value, err = draw.NewDrawFromFile(with, height, path)
...
func checkDraw(L *lua.LState) *draw.Draw {
	ud := L.CheckUserData(1)
	if v, ok := ud.Value.(*draw.Draw); ok {
		return v
	}

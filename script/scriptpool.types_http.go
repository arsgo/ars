...
func (s *ScriptPool) getHttpTypeBinder() script.LuaTypesBinder {
...
func typeNewHTTP(L *lua.LState) int {
	...
	case 1:
		...
		http = net.NewHTTPClientProxy(proxy)
	case 3:
		certFile := L.CheckString(1)
		keyFile := L.CheckString(2)
		caFile := L.CheckString(3)
		http, err = net.NewHTTPClientCert(certFile, keyFile, caFile)
		if err != nil {
...
func checkHTTP(L *lua.LState) *net.HTTPClient {
	ud := L.CheckUserData(1)
	if v, ok := ud.Value.(*net.HTTPClient); ok {
		return v
	}
